...
- [ ] ⏭️ **TOP PRIORITY — both frontier charts have the wrong axes, not just one. Two sub-items, both unsolved, both design-noted in [12-weighted-quality-score-notes.md](12-weighted-quality-score-notes.md):**
  - [ ] **Quality axis:** a real weighted quality score, not range/mileage as a stand-in. Concrete evidence this is blocking: on the used price-vs-mileage chart ([06](06-secondhand-price-vs-mileage.html)), the Pareto frontier collapses to just two points — BYD Dolphin Surf (100km, €19,990) and Dacia Spring (40,517km, €7,500) — with the Spring dominating every other car on price alone out to 160,000km. Pure price-vs-mileage says "buy the Spring." Everything else in this project says don't (1★ Euro NCAP). The frontier logic needs to know *why* the Dolphin Surf costs ~€12.5k more for similar-ish WLTP range: safety score, charging speed, battery chemistry/thermal management, interior quality, warranty — none of which the current range/mileage axes can see. Same problem on the new-car frontier in [03](03-price-range-sketch.html) (Spring also anchors that one).
  - [ ] **Price axis: sticker price is the wrong number — should be annualized cost of ownership** (depreciation/yr + running costs/yr + risk-adjusted damage/repair cost/yr). Worked example of why this matters: a €60,000 car depreciating €2,000/yr genuinely beats a €30,000 car depreciating €3,000/yr on true cost of ownership, even though it costs twice as much upfront — sticker price gets that backwards. Every frontier in this project currently plots sticker price. Needs a real per-model depreciation *curve* (not just the aggregate "50-60% over 3yr" figure in [02](02-model-tco-survey.md)), running costs extended beyond the budget shortlist, and a damage/repair-risk proxy that doesn't exist in this project yet.
  
  **Not solving either now — just flagging both as the next real piece of work**, ranking above the other open items below until picked up.
...
- `12-weighted-quality-score-notes.md` — appendix: design notes for a real price/quality frontier (both the quality axis and the price→annualized-cost axis), not yet built
