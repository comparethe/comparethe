...
- [x] Restyled index.md's verdict section as HTML pick-cards (matching the "Buy a Car" hub's visual language: CSS custom-property theme tokens, light/dark aware) with inline SVG car-silhouette icons in place of the old plain markdown table. No real photos used — licensing/hotlinking risk for a published site, and inline SVG keeps it self-contained and theme-consistent.
